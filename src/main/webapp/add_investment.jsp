...
<h4 class="widgettitle">Add Investment</h4>
...
<%
try{	
	investorList=new ProjectDAO().getInvestorList();
	boolean status=false;
	for(int i=0;i<investorList.size();i++){
    status=new ProjectDAO().checkInvestment(investorList.get(i).split(":")[1],session.getAttribute("project_id").toString());
%>	
<%if(status){ %>				
<option value="<%=investorList.get(i).split(":")[1]%>" style="color: yellow;" disabled="disabled"><%=investorList.get(i).split(":")[0]%></option>
<%}else{ %>
<option value="<%=investorList.get(i).split(":")[1]%>"><%=investorList.get(i).split(":")[0]%></option>
<%}
}
}catch(Exception e){
	e.printStackTrace();
...
%>
					  ...
					  <tr>
						...
						<td>
							...
							<div>
							<input type="text" name="pro_amount" id="pro_amount" class="" value="" required="required"/>
							</div>
						</td>
					  </tr>
					 					  <tr>
					...
					<br>
					<br>
						<input type="hidden" name="todo" value="addInvestment">
